...
module Plush.DocTest.Posix (
    readProcessCombinedWithExitCode
    )
    where
...
-- | Just like 'readProcessWithExitCode' from "System.Process", only @stdout@
-- and @stdin@ are opened on the same 'Fd' so their output is combined.
readProcessCombinedWithExitCode
    :: FilePath                 -- ^ command to run
    -> [String]                 -- ^ any arguments
    -> String                   -- ^ standard input
    -> IO (ExitCode, String)    -- ^ exitcode, stdout and stderr
readProcessCombinedWithExitCode cmd args input = do
