...
--
-- HiEndOfOutput is synthesized when sending the output to the client, and is
-- not stored in a history file. Hence, it is not generated by parseJSON.
--
-- JSON serialized simply as each variant's JSON serialization
...
instance ToJSON HistoryItem where
    ...
    toJSON HiEndOfOutput = object [ "endOfOutput" .= True ]
